...
from .triplet_loss import WeightedRegularizedTriplet
...
def make_loss(cfg, num_classes):  # modified by gu
    ...
    center_criterion = CenterLoss(num_classes=num_classes,
                                  feat_dim=feat_dim,
                                  use_gpu=True)  # center loss
    if 'triplet' in cfg.MODEL.METRIC_LOSS_TYPE:
        if cfg.MODEL.WRTRIPLET:
            triplet = WeightedRegularizedTriplet()
            print("using weighted regularized triplet loss for training")
        else:
            if cfg.MODEL.NO_MARGIN:
                ...
                print("using triplet loss with margin:{}".format(
                    cfg.SOLVER.MARGIN))
    else:
