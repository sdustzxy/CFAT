...
class Backbone(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        self.normalize = NormalizeByChannelMeanStd(mean=cfg.INPUT.PIXEL_MEAN,
        ...
        self.gem_pool = cfg.MODEL.GEM_POOL
        if self.gem_pool:
    ...
    def forward(self,
                x,
                label=None, return_featuremaps=False,
                **kwargs):  # label is unused if self.cos_layer == 'no'
...
class build_transformer(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory):
        ...
        self.base = factory[cfg.MODEL.TRANSFORMER_TYPE](
            ...
            linear_block=cfg.MODEL.LINEAR_BLOCK)
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None):
        x = self.normalize(x)
        global_feat, feat_ori, feat_dec = self.base(x, cam_label=cam_label, view_label=view_label)
        ...
        if self.training:
            ...
            return cls_score, global_feat, feat_ori, feat_dec  # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return feat, feat_ori
            else:
                # print("Test with feature before BN")
                return global_feat, feat_ori
...
def make_model(cfg, num_class, camera_num, view_num):
    if cfg.MODEL.NAME == 'transformer':
        if cfg.MODEL.JPM:
            model = build_transformer_local(num_class,
                                            camera_num,
                                            view_num,
                                            cfg,
                                            __factory_T_type,
