...
from model.backbones.timm.models.layers import Mlp, DropPath, trunc_normal_
...
class PixelEmbed(nn.Module):
    ...
    def forward(self, x, pixel_pos):
        ...
        x = self.proj(x)
        x = self.unfold(x)
        x = x.transpose(1, 2).reshape(B * self.num_patches, self.in_dim,
...
def _create_tnt(variant, pretrained=False, **kwargs):
    ...
    model = build_model_with_cfg(TNT,
                                 variant,
                                 pretrained,
...
def tnt_s_patch16_224(pretrained=False, **kwargs):
    model_cfg = dict(patch_size=16,
                     ...
                     qkv_bias=False,
                     **kwargs)
    model = _create_tnt('tnt_s_patch16_224',
...
def tnt_b_patch16_224(pretrained=False, **kwargs):
    model_cfg = dict(patch_size=16,
                     ...
                     qkv_bias=False,
                     **kwargs)
    model = _create_tnt('tnt_b_patch16_224',
                        pretrained=pretrained,
                        **model_cfg)
    return model
