...
class AggregationRefpad(Function):
    ...
    def forward(ctx, input, weight, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('aggregation_refpad_forward_kernel',
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                n = grad_input.numel()
                opt['nthreads'] = n
                f = load_kernel('aggregation_refpad_input_backward_kernel',
            ...
            if ctx.needs_input_grad[1]:
                ...
                opt['nthreads'] = n
                f = load_kernel('aggregation_refpad_weight_backward_kernel',
...
def aggregation_refpad(input,
                       weight,
                       kernel_size=3,
                       stride=1,
                       padding=0,
                       dilation=1):
    assert input.shape[0] == weight.shape[0] and (input.shape[1] %
                                                  weight.shape[1] == 0)
    if input.is_cuda:
        out = AggregationRefpad.apply(input, weight, kernel_size, stride,
                                      padding, dilation)
    else:
...
def test_aggregation_refpad():
    ...
    out_width = int((in_width + 2 * padding -
                     (dilation * (kernel_size - 1) + 1)) / stride + 1)
    x = torch.randn(n, c_x, in_height, in_width,
    ...
    x2 = unfold_j(pad(x)).view(n, c_x // c_w, c_w, pow(kernel_size, 2),
                               out_height * out_width)
    ...
    assert torch.autograd.gradcheck(
        partial(aggregation_refpad,
                kernel_size=kernel_size,
                stride=stride,
                padding=padding,
                dilation=dilation), (x, w))
