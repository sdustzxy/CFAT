...
class Subtraction2Refpad(Function):
    ...
    def forward(ctx, input1, input2, kernel_size, stride, padding, dilation):
        ...
        output = input1.new(batch_size, input_channels,
                            kernel_size[0] * kernel_size[1],
                            output_height * output_width)
        n = output.numel() // output.shape[2]
        with torch.cuda.device_of(input1):
            f = load_kernel('subtraction2_refpad_forward_kernel',
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input1):
            if ctx.needs_input_grad[0]:
                ...
                n = grad_input1.numel()
                opt['nthreads'] = n
                f = load_kernel('subtraction2_refpad_input1_backward_kernel',
                                _subtraction2_refpad_input1_backward_kernel,
                                **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
                  args=[grad_output.data_ptr(),
                        grad_input1.data_ptr()],
                  stream=Stream(ptr=torch.cuda.current_stream().cuda_stream))
        with torch.cuda.device_of(input2):
            if ctx.needs_input_grad[1]:
                grad_input2 = input2.new(batch_size, input_channels,
                                         input_height + 2 * padding[0],
                                         input_width + 2 * padding[1])
                n = grad_input2.numel()
                opt['nthreads'] = n
                f = load_kernel('subtraction2_refpad_input2_backward_kernel',
                                _subtraction2_refpad_input2_backward_kernel,
                                **opt)
                f(block=(CUDA_NUM_THREADS, 1, 1),
                  grid=(GET_BLOCKS(n), 1, 1),
                  args=[grad_output.data_ptr(),
                        grad_input2.data_ptr()],
                  stream=Stream(ptr=torch.cuda.current_stream().cuda_stream))
                grad_input2[:, :, padding[0] + 1:2 * padding[0] +
                            1, :] += torch.flip(
                                grad_input2[:, :, :padding[0], :], dims=[2])
                grad_input2[:, :, input_height - 1:input_height + padding[0] -
                            1, :] += torch.flip(
                                grad_input2[:, :,
                                            input_height + padding[0]:, :],
                                dims=[2])
                grad_input2[:, :, :, padding[1] + 1:2 * padding[1] +
                            1] += torch.flip(grad_input2[:, :, :, :padding[1]],
                                             dims=[3])
                grad_input2[:, :, :, input_width - 1:input_width + padding[1] -
                            1] += torch.flip(grad_input2[:, :, :, input_width +
                                                         padding[1]:],
                                             dims=[3])
...
def subtraction2_refpad(input1,
                        input2,
                        kernel_size=3,
                        stride=1,
                        padding=0,
    ...
    assert input1.dim() == 4
    if input1.is_cuda:
        out = Subtraction2Refpad.apply(input1, input2, kernel_size, stride,
                                       padding, dilation)
    else:
...
def test_subtraction2_refpad():
    ...
    out_width = int((in_width + 2 * padding -
                     (dilation * (kernel_size - 1) + 1)) / stride + 1)
    x1 = torch.randn(n, c, in_height, in_width,
                     requires_grad=True).double().cuda()
    x2 = torch.randn(n, c, in_height, in_width,
                     requires_grad=True).double().cuda()

    y1 = subtraction2_refpad(x1,
                             x2,
    ...
    y2 = unfold_i(x1).view(n, c, 1, out_height * out_width) - unfold_j(
        pad(x2)).view(n, c, pow(kernel_size, 2), out_height * out_width)
    ...
    assert torch.autograd.gradcheck(
        partial(subtraction2_refpad,
                kernel_size=kernel_size,
                stride=stride,
                padding=padding,
                dilation=dilation), (x1, x2))
