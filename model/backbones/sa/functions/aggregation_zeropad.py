...
class AggregationZeropad(Function):
    ...
    def forward(ctx, input, weight, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('aggregation_zeropad_forward_kernel',
...
def aggregation_zeropad(input,
    ...
    assert input.shape[0] == weight.shape[0] and (input.shape[1] %
                                                  weight.shape[1] == 0)
    if input.is_cuda:
        out = AggregationZeropad.apply(input, weight, kernel_size, stride,
                                       padding, dilation)
    else:
...
def test_aggregation_zeropad():
    ...
    unfold_j = torch.nn.Unfold(kernel_size=kernel_size,
                               ...
                               padding=padding,
                               stride=stride)
    x2 = unfold_j(x).view(n, c_x // c_w, c_w, pow(kernel_size, 2),
                          out_height * out_width)
