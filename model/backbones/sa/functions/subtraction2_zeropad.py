...
class Subtraction2Zeropad(Function):
    ...
    def forward(ctx, input1, input2, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input1):
            f = load_kernel('subtraction2_zeropad_forward_kernel',
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input1):
            if ctx.needs_input_grad[0]:
                ...
                n = grad_input1.numel()
                opt['nthreads'] = n
                f = load_kernel('subtraction2_zeropad_input1_backward_kernel',
        ...
        with torch.cuda.device_of(input2):
            if ctx.needs_input_grad[1]:
                ...
                n = grad_input2.numel()
                opt['nthreads'] = n
                f = load_kernel('subtraction2_zeropad_input2_backward_kernel',
...
def subtraction2_zeropad(input1,
                         input2,
                         kernel_size=3,
                         stride=1,
                         padding=0,
    ...
    assert input1.dim() == 4
    if input1.is_cuda:
        out = Subtraction2Zeropad.apply(input1, input2, kernel_size, stride,
                                        padding, dilation)
    else:
...
def test_subtraction2_zeropad():
    ...
    unfold_j = torch.nn.Unfold(kernel_size=kernel_size,
                               ...
                               stride=stride)
    y2 = unfold_i(x1).view(
        n, c, 1, out_height * out_width) - unfold_j(x2).view(
            n, c, pow(kernel_size, 2), out_height * out_width)
