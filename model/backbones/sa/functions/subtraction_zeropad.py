...
class SubtractionZeropad(Function):
    ...
    def forward(ctx, input, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel('subtraction_zeropad_forward_kernel',
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            if ctx.needs_input_grad[0]:
                ...
                n = grad_input.numel()
                opt['nthreads'] = n
                f = load_kernel('subtraction_zeropad_input_backward_kernel',
...
def subtraction_zeropad(input, kernel_size=3, stride=1, padding=0, dilation=1):
    assert input.dim() == 4
    if input.is_cuda:
        out = SubtractionZeropad.apply(input, kernel_size, stride, padding,
                                       dilation)
    else:
